...
| `thesis-print.tex` | Root of the print-ready variant: sets `\ThesisPrintVariant` and `\input`s `thesis.tex` unchanged. |
| `thesis-style.sty` | Thesis-wide style package; defines `\paperchapter` and the CV formatting macros used by `front-matter/cv.tex`, and holds every `\ifpc@print` branch. |
...
- **Print variant.** `thesis-print.tex` defines `\ThesisPrintVariant` and then `\input`s `thesis.tex` verbatim; `thesis-style.sty` turns that into the `\ifpc@print` flag and is the *only* file in the build that branches on it — keep it that way, so the two PDFs never diverge in content. Three differences: `hyperref` gets `hidelinks` instead of `colorlinks` (so cross-refs, citations, URLs, ToC and bibliography DOIs all print black, while the PDF keeps working bookmarks); `\href` is redefined at `\AtEndPreamble` to `#3\footnote{\url{#2}}` so external URLs are readable on paper; and `\cvpersonal` branches by hand, because its four `\href`s sit in a `tabular` where `\footnote` is swallowed. Use `\pc@printalt{print}{digital}` rather than a bare `\ifpc@print` wherever a leftover `\if`/`\fi` would be unsafe (tabular cells, `\setlength` arguments). Colors that carry information — figure PDFs, the TikZ palette, the color-keyed legend words in `chapters/exponents.tex` and `papers/2506/` — are deliberately left alone.
- **Don't add a `geometry` `bindingoffset` to the print variant.** It was tried and dropped. On fixed A4 with a fixed 140mm measure the inner and outer margins always sum to 70mm, so a spine gutter can only be funded out of the outer margin — where the 37mm `\addnote` apparatus already sits. A 5mm offset re-splits the remaining 65mm as 26:39 and leaves margin notes 2mm from the page edge, inside a typical printer's non-printable border. The full reasoning is in the margin-budget comment in `thesis-style.sty`.
...
- Printable thesis: `latexmk -outdir=build/print thesis-print.tex`. The `-outdir` is **required** — `\include{chapters/foo}` writes `chapters/foo.aux`, named after the included file rather than the jobname, so a shared `build/` would let the two thesis builds clobber each other's aux. `latexmkrc` pre-creates both aux trees.
- All input paths are resolved **relative to the repo root** (the CWD you run `latexmk` from), never relative to the including file or to `$out_dir`.
