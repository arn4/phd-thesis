...
From Eq.~\eqref{2305:eq:risk_square} when the weights are on the sphere, it follows that the risk is given by
\begin{equation}
  \risk{(\Q,\M)}-\frac\Delta2 = 1 + \frac1p + \frac{1}{p^2}\sum_{j,l=1;j\neq l}^p \Q^2_{jl} - \frac2p \sum_{j=1}^p \M^2_{j}
\end{equation}
Eqs.~\eqref{2305:eq:approx_suff_stat} can be used to obtain a deterministic time evolution of the risk. The only source of randomness left is from the initial conditions, we can define two random variables as
\begin{equation}
...
Wrapping all this consideration together, Eq.~\eqref{2305:eq:exit_time_implicit_equation} is
\begin{equation}
...
In this case \(\tau_0\) does not appear, and from Eq.~\eqref{2305:eq:initial_distribution_QM} we find that \(\mu_0 \sim \chi^2(1)\), so the quenched formula reduces to
\begin{equation}
