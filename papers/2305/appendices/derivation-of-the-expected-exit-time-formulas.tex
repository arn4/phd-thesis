...
\label{2305:app:exit_time-derivation}
...
\begin{equation}\begin{split}
  ...
  &\qquad\frac1{p^2}\left[\left(p^2+2p\right)\Q_{jl} +
        8p \Q_{jl} + 24\Q_{jl} \right] + \Delta \Q_{jl}\Biggr\} \\
  &= -\frac{16}{p} \Q_{jl} \\
    &\quad+\frac{4 \gamma}{p} \left\{
      3 -2 -\frac{8}{p} +1 +\frac2p +\frac8p + \frac{24}{p^2} + \Delta \right\}\Q_{jl} \\
  &= -\frac{16}{p} \Q_{jl} \\
  &\quad+\frac{4 \gamma}{p} \left\{2 +\frac{2}{p} + \frac{24}{p^2} + \Delta \right\}\Q_{jl}
...
\begin{equation}\label{2305:eq:Msphericalgenericpk1}\begin{split}
  \dod{\left[\M{\left(t\right)}\right]_{j}}{t} &= 
    ...
    \right] \left[\M{\left(t\right)}\right]_{j} ,
\end{split}\end{equation}
\begin{equation}\label{2305:eq:Qsphericalgenericpk1}\begin{split}
...
\begin{equation}\begin{split} \label{2305:eq:approx_suff_stat}
...
From Equation~\eqref{2305:eq:risk_square} when the weights are on the sphere, it follows that the risk is given by
\begin{equation}
  \risk{(\Q,\M)}-\frac\Delta2 = 1 + \frac1p + \frac{1}{p^2}\sum_{j,l=1;j\neq l}^p \Q^2_{jl} - \frac2p \sum_{j=1}^p \M^2_{j}
\end{equation}
Equations~\eqref{2305:eq:approx_suff_stat} can be used to obtain a deterministic time evolution of the risk. The only source of randomness left is from the initial conditions, we can define two random variables as
\begin{equation}
...
\begin{equation}
    \risk(t)-\frac\Delta2 = 1+\frac1p+\frac{\tau_0}{dp^2}\exp[-2\omega_Q t]-\frac{2\mu_0}{dp}\exp[2\omega_M t]
\end{equation}
...
Wrapping all this consideration together, Equation~\eqref{2305:eq:exit_time_implicit_equation} is
\begin{equation}
...
\begin{equation}\begin{split}
    t_\text{ext}^\text{(qnc)} &= \mathbb E_{\mu_0,\tau_0 \sim \mathcal{P}^d_p} \\
    &\quad\left[
...
In this case \(\tau_0\) does not appear, and from Equation~\eqref{2305:eq:initial_distribution_QM} we find that \(\mu_0 \sim \chi^2(1)\), so the quenched formula reduces to
\begin{equation}
...
\subsection{Overparameterization Gain} \label{2305:app:overp_gain}
Let us introduce the number of gradient steps needed to escape the threshold. Remembering \(t=\sfrac{\nu\gamma}{pd}\), we define
\begin{equation}
    s_\text{ext}{(p,d,\gamma,\Delta,T)} \coloneqq \frac{pd}{\gamma} t_\text{ext}.
\end{equation}
In the domain of our interest, namely \(s_\text{ext}>0\),  \(s_\text{ext}\) is a convex function in \(\gamma\). Therefore, there exists a unique minimum that corresponds to the minimum number of steps required to cross the threshold when \(p, d\) are fixed
\[
...
We can also compute the optimal number of steps; we choose to stick with the \emph{annealed formula}: for large \(p\) both estimations lead to the same result, while for small \(p\) we are underestimating the exit time of a small factor. Hence, the annealed minimum number of steps is
\[\begin{split}
...
\end{split}\]
This formula can be used to estimate the overparameterization gain. Noting that \(s^\text{min}_\text{ext}\) is monotonically decreasing in \(p\), we can define the gain as
\[
