...
We can now take the limit \(d\to+\infty\), claiming that the theorem~\cite{veiga2022phase,goldt2019dynamics} proving ODE convergence is still valid. Indeed, the error term $o(d^{-1})$ in Eq. \eqref{2305:eq:app:m_update_process} has an average order of $O(d^{-2})$, which can be absorbed in the term $\Gamma^\nu$ of Theorem A.1 in \cite{veiga2022phase}. The rest of the proof proceeds the same way, noting that the square function can be assumed to be Lipschitz since the dynamics take place on the sphere.
