...
\end{split}\end{equation}
Applying the definition of the sufficient statistics, \(m_j = \sfrac{\w_j\w_\star}{d}\) and \(\Q_{jl} = \sfrac{\w_j\w_l}{d}\), we can recover Eqs.~\eqref{2305:eq:def:overlap_process}.
...
While the full risk expression can be found in Eq.~\eqref{2305:eq:def:risk}, we report here just the case \(a_j=1\) for the ODEs since they have a compact matrix form
...
For completeness, this is Eq.~\eqref{2305:eq:def:risk} for the case \(a_j=1\)
