\section{Stochastic correction to the exit-time formula}

\label{2305:app:SDE_exit_time} 
...
At first, we require the further assumption \(w^0\perp\w_\star^0\). Obviously, this is not realistic, since to achieve this initialization we should know \(\w_\star\) exactly. Still, this case is could arise interest, since it corresponds to \(m^0=0\), that is a fixed point of Equation~\eqref{2305:eq:spherical-p1-ode}. Thus, there is no dynamics in the ODE description, while the SDE are able to jump out from the fixed point and reach the point of convergence of the SGD. Lastly, the following analysis can be generalized to be used with the usual initial conditions.

The key observation is approximating both the noise and the drift of Equation~\eqref{2305:eq:constrained_brownian_motion_p1} to the first non-zero order, when evaluated at \(m=0\). As we pointed out above, the drift term is null at initialization, so the leading order is the first; this corresponds to the linearization of Equation~\eqref{2305:eq:spherical-p1-ode}, and the linear factor is
\begin{equation}
    \mu \coloneqq \left[4(1-6\gamma)-2\gamma\Delta\right].
\end{equation}
Instead, the noise term is not vanishing at \(m=0\). Of course, \(\vec{\sigma}_\Q{(\Omega)}\) does not contribute because is proportional to \(m_S\). Moreover, looking at the expression for \(\Sigma\) in Appendix~\ref{2305:app:SDE_with_generic_width}, we can observe that the covariance matrix is diagonal since the non-diagonal term is proportional to \(m_S\) as well. All considered, the only term is contributing is the variance of \(m\): the two-dimensional Brownian motion can be reduced to one in dimension 1, with variance
\[
...
\begin{equation} \label{2305:eq:exit_time_SDE}
...
As reported in \cite{zeng2020mean}, the formula can be generalized also to the initial condition we used in the rest of the manuscript, where \(\M \sim \mathcal N \left(0,\sfrac{1}{d}\right)\). Again, we have to average over the initial conditions: \eqref{2305:eq:exit_time_SDE} coincides with the \emph{annealed} version, while we do not present here the \emph{quenched} one to be concise.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
