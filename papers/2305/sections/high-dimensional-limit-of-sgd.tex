...
\paragraph{Sufficient statistics ---} The key observation is to notice that the population risk only depends on the hidden-layer weights $W\in\mathbb{R}^{p\times d}$ through the second layer weights \(a\in\mathbb{R}^p\) and the weights' correlation matrices:
...
\begin{equation}\label{2305:eq:def:risk}
    \begin{split}
...
Notice that the matrices $M, Q$ are precisely the second moments of the pre-activations $(\lambda_{\star},\lambda) = (w_{\star}^{\top}x, Wx)\in\mathbb{R}^{p+1}$. Therefore, to characterize the evolution of the risk throughout SGD, it is sufficient to track the evolution of the second layer weights $a_{i}$ and the correlation matrices $m, Q$, which consists of $p(p+1)$ parameters. As shown in Appendix~\ref{2305:app:square_equations}, starting from Equation~\eqref{2305:eq:sgd} we can derive a set of self-consistent stochastic processes describing the evolution of $(a,m,Q)$:
\begin{align}
\label{2305:eq:def:overlap_process}
    a_{j}^{\nu+1}-a_{j}^{\nu} &= \frac{\gamma}{pd}\mathcal{E}^{\nu}\lambda_{j}^2 \\
    m_{j}^{\nu+1}-m_{j}^{\nu} &= \frac{\gamma}{pd}\,\mathcal M_j(a,\lambda_{\star},\lambda), \qquad \mathcal M_j(a,\lambda_{\star},\lambda)\coloneqq 2\mathcal{E}^{\nu}a_{j}\lambda_{j}\lambda_{\star}\\
    Q_{jl}^{\nu+1} - Q^{\nu}_{jl} &= \frac{\gamma}{pd}\,\mathcal Q_{jl}(a,\lambda_{\star},\lambda), \qquad \mathcal Q_{jl}(a,\lambda_{\star},\lambda)\coloneqq 2\mathcal{E}^{\nu}\left(a_{j}+a_{l}\right)\lambda_{j}\lambda_{l}+4\frac{\gamma}{p}{\mathcal{E}^{\nu}}^{2}||x^{\nu}||^2 a_{j}a_{l}\lambda_{j}\lambda_{l}
\end{align}
where we have defined the displacement vector 
\begin{equation}
\mathcal{E}^{\i} \coloneqq (\lambda^{\star\i})^2 - \frac{1}{p}\sum_{j=1}^{\hids}a_{j}(\lambda_{j}^{\nu})^2 + \sqrt{\noise} z^{\i},
\end{equation}
...
\end{align}
When $d\to\infty$, the weights are being orthogonal with high probability. In terms of the sufficient statistics: 
