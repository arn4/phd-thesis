...
In Figure~\ref{2405:fig:app:other-algos}~Left, we repeat one of the experiment of Figure~\ref{2405:fig:main:single_index_complete} in the case \(\rho<0\), the \emph{Sharpness-Aware Minimization}. The two algorithms are based on very different principle, but in our context they behave in the same way: the reusing of the data makes high information exponent function easily learnable. The global picture for \(\rho\neq0\) is the same regardless of its sign. 
...
\begin{align}
    T \cdot n_b \sim 
        \begin{cases}
        &O(d^{\ell^\star -1} ) \qquad \text{if $\ell^\star>2$} \\
