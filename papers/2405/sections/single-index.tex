...
In this section we rigorously characterize the number of iterations needed for the class of Algorithms~\ref{2405:algo:optimizer_main_step} to perform weak recovery (as in Definition~\ref{2405:def:weak_recovery}) in the context of single-index targets. We establish a clear separation between the learning efficiency of the algorithmic  family~\ref{2405:algo:optimizer_main_step} and One-Pass SGD, limited by the Information Exponent of the target to be learned \citep{arous2021online}.

We start by introducing a restriction of the generative exponent in \cite{damian2024computational} to polynomial transformations.
\begin{definition}[Polynomial Generative Exponent]
\label{2405:def:lstar_damian2024}
Furthermore, we define $\ell^\star_p$ as the smallest integer $k$ such that there exists a polynomial $p: \R \rightarrow \R$ with:
...
\begin{assumption}\label{2405:assump:spherical_corr}
    ...
    We also consider a spherical version of Algorithm~\ref{2405:algo:optimizer_main_step}, with
...
Assumption \ref{2405:assump:spherical_corr} simplifies Algorithm\ref{2405:algo:optimizer_main_step} to a more tractable version. In particular, the normalization step allows us to keep track of only one parameter, the overlap between $\vec{w}_t$ and $\vec{w}^\star$. We expect however Theorem~\ref{2405:thm:single_index_recovery} to hold in the general setting of Algorithm~\ref{2405:algo:optimizer_main_step}.
...
\paragraph {Learning polynomial functions --} Although our definition of the Polynomial Generative Exponent is more restrictive than the one in \cite{damian2024computational}, we show that it is enough to allow learning of a large class of functions, namely polynomials:
\begin{theorem}\label{2405:thm:poly_pge}
    Assume that $h^\star$ is a polynomial function. Then the Polynomial Generative Exponent of $h^\star$ is always at most 2, and is equal to 1 whenever $h^\star$ is not even.
\end{theorem}
...
\begin{figure}
    \centering
    \subfigure[$h^\star(x) = \operatorname{relu}(x)$]{%
        \includegraphics[height=0.45\textwidth]{figs/fig1_ReluRelu}
        \label{2405:fig:single_index_1}
    }
    \hspace{-4mm}
    \subfigure[$h^\star(x) = \mathrm{He}_2(x)$]{%
        \includegraphics[height=0.45\textwidth]{figs/fig1_H2Relu}
        \label{2405:fig:single_index_2}
    }
    %
    \subfigure[$h^\star(x) = \mathrm{He}_3(x)$]{%
        \includegraphics[height=0.45\textwidth]{figs/fig1_H3Relu}
        \label{2405:fig:single_index_3}
    }
    \hspace{-4mm}
    \subfigure[$h^\star(x) = \mathrm{He}_4(x)$]{%
        \includegraphics[height=0.45\textwidth]{figs/fig1_H4Relu}
    ...
    (Details in Appendix~\ref{2405:sec:app:implementation}).}
    \label{2405:fig:main:single_index_complete}
\end{figure}
We illustrate in Figure~\ref{2405:fig:main:single_index_complete} the stark difference between the weak recovery dynamics of One-Pass SGD and Algo.~\ref{2405:algo:optimizer_main_step} for single-index targets. The Cosine Similarity between the learned weight $\vec w_t$ and the ground truth informative direction $\vec w_\star$ is shown as a function of the time steps $t$ for different target functions. Two optimization routines are considered to exemplify the learning behavior:
\begin{enumerate}
    \item Extragradient Descent (EgD), corresponding to the family of Algorithms~\ref{2405:algo:optimizer_main_step} with positive $\rho$ parameters;
    \item One-Pass SGD, corresponding to vanilla SGD, or equivalently Algorithm~\ref{2405:algo:optimizer_main_step} associated with $\rho=0$ hyperparameter.
\end{enumerate}The scaling of $(\gamma, \rho)$ as a function of the input dimension  $d$ and exponent $\ell^\star$ are given in Theorem~\ref{2405:thm:single_index_recovery}, while the mini-batch size is fixed to $n_b =1$ (See Appendix~\ref{2405:sec:app:numerics} for extension to larger $n_b$). For all plots, we take $\sigma=\mathrm{relu}$ and we refer to Appendix~\ref{2405:sec:app:implementation} for details.
\begin{itemize}
    \item \textbf{SGD-easy non-symmetric targets ($\ell = \ell^\star =1$) --} The Left section of Figure~\ref{2405:fig:main:single_index_complete} shows $h^\star(x) = \mathrm{relu}(x)$. Here both SGD and Algorithm~\ref{2405:algo:optimizer_main_step} learn in $T= O(d)$.
    \item \textbf{SGD-easy symmetric targets ($\ell = \ell^\star = 2$) -- } The Center-Left section of Figure~\ref{2405:fig:main:single_index_complete} shows $ h^\star(x) = \mathrm{He}_2(x)$. Here both SGD and Algorithm~\ref{2405:algo:optimizer_main_step} learn in $T=O(d \log d)$. 
    \item \textbf{SGD-hard non-symmetric targets ($\ell > 2, \ell^\star =1$) -- }  The Center-Right section of Figure~\ref{2405:fig:main:single_index_complete} shows $ h^\star(x) = \mathrm{He}_3(x)$. Here Algortihm~\ref{2405:algo:optimizer_main_step} learns in $T=O(d)$ while One Pass SGD suffers from the limitations detailed in Equation~\eqref{2405:eq:gba_time_scaling}, i.e. $T= O(d^{\ell-1})$ with $\ell=3$ for this case.
    \item \textbf{SGD-hard symmetric targets ($\ell > 2, \ell^\star =2$) -- } The Right section of Figure~\ref{2405:fig:main:single_index_complete} shows $h^\star(x) = \mathrm{He}_4(x)$. Here Algortihm~\ref{2405:algo:optimizer_main_step} learns in $T=O(d \log d)$ while One Pass SGD suffers from the time scaling law detailed in Equation~\eqref{2405:eq:gba_time_scaling} $T= O(d^{\ell-1})$.
\end{itemize}
