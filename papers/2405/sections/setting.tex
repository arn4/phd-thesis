...
\end{align}
We consider target functions that are dependent only on few orthogonal relevant directions $k=O_d(1)$ encoded in the target's weight matrix $W^\star = \{\vec w^\star_r\}_{r \in [k]} \in \mathbb{R}^{k\times d}$:\addnote{The target $f^\star$ plays the role of the teacher and the network $f$ that of the student from Section~\ref{intro:sec:settings}; we switch to \emph{target}/\emph{network} here because $f^\star$ and $f$ no longer share the same architecture.}
\begin{align}
...
This objective is non-convex, and we do not have direct access to the expectation over the data. A central question in Machine Learning is therefore to quantify the properties of the weights attained at convergence by different algorithms. In this manuscript we consider a specific class of one-pass training routines to minimize the empirical risk. First, we partition the dataset in disjoint mini-batches of size $n_b$, i.e. $\mathcal{D} = \cup_{t \in [T]}\{\vec z^\nu, y^\nu\}_{\nu \in \{tn_b+1, \dots, (t+1)n_b\}}$ where $T$ is the total number of iterations considered.\addnote{This chapter departs from the discrete/continuous time convention of Section~\ref{intro:sec:settings}: here $t$ denotes the discrete SGD step (called $\nu$ there), since $\nu$ is reserved for indexing the individual samples $\vec z^\nu$.} For each algorithmic step, we update the hidden layer neurons $W=\{\vec w_j\}_{j \in [p]}$ with the program $\mathcal{A}: (\vec w_{j,t}, \gamma, \rho, n_b) \in \mathbb{R}^{d+3} \to \vec w_{t+1,j} \in \mathbb{R}^d$.
...
We discuss in this work the extension of the generative exponent to the multi-index setting and investigate the dynamics of specific gradient-based programs (Algorithm~\ref{2405:algo:optimizer_main_step}) as a function of the freshly defined hardness exponent $\ell^\star$.
...
\end{theorem}
This shows in particular that a simple gradient-based algorithm, which only requires seeing the data twice, is optimal---in terms of SGD performance---for weak recovery of any polynomial target.
