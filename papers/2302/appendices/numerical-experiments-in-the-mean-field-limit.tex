...
Second, we test whether it is actually necessary to compute the expectation over the matrix \(\Xi\) when integrating the mean field in low dimension.
...
\begin{figure}
    ...
    \caption{
     (a) Evolution from starting point \(Q^\bot = 0\), tracking only \(\M\). (b) - (c)  Low-dimension simulations and the corresponding ODE trajectory, without the off-diagonal entries of matrix $\Xi$ (d) Histogram of the student-teacher weight overlaps at different times.
    }
