...
\subsection{Proof of Eq. \eqref{2302:eq:mf:concentration}}
...
We are now in a position to show Eq. \eqref{2302:eq:mf:concentration}. For brevity, we denote by $\mathbb E$ (resp. $\mathbb E_{\text{MF}}$) the expectations with respect to $\vec\lambda^\bot \sim \mathcal N(\vec{0}, Q^\bot)$ (resp. $\vec\lambda^\bot \sim \mathcal N(\vec{0}, \mathrm{diag}(Q^\bot))$). Since the marginals of both distributions are the same, and by linearity,
...
\end{equation}
and Eq. \eqref{2302:eq:mf:concentration} ensues by taking the expectation w.r.t $\vec{\lambda}^\star$ on both sides.
