...
\paragraph{Data model: } Stochasticity in eq.~\eqref{eq:def:sgd} is induced by the draw of samples $(\vec{x}, y)\sim \rho$. In the following, we will assume that the input $\vec{x}^\i$ are Gaussian $\vec{x}^{\i}\sim\mathcal{N}\left(\vec{0}_{d}, \sfrac{1}{d}\mat{I}_{d}\right)$ while $y^\i$ is drawn from the following generative model:
\begin{align}
\label{eq:target}
...
\begin{assump}\label{assump:activation}
...
\end{assump}
Note that in some plots, we will sometimes use the function $\sigma(x) = x^2$, which does not satisfy this assumption. However, Assumption \ref{assump:boundedness} ensures that we stay in a bounded subset of $\mathbb R^d$, hence we can replace $\sigma$ by $\sigma \wedge K$ for $K$ sufficiently large.
