...
\subsection{Computing the gradient at time \texorpdfstring{$t$}{t}}

A direct computation of both gradients implies the following lemma:
\begin{lemma}
Define
...
Hence, the main difference between the gradients of the correlation and square loss is a so-called \emph{self-interaction term} $\hat{\vec{g}}$, that only depends on the learned vector $\vec{w}_t$. Notice that $\vec{g}_t^\star$ is an average of $n_b$ independent variables of the form
\begin{equation}
...
\subsection{A differential inequality for \texorpdfstring{$m_t$}{m\_t}}
...
\begin{lemma}\label{2406:lem:app:bound_corrections}
    ...
    \begin{equation}
        \mathbb{P}\left(\norm{\vec{g}_t^\bot}^2 \leq C\left(\phi(m_t)^2(1 - m_t^2)^2  + \frac {d\log(d)^C} {n_b}\right) \quad \forall t \leq t_\eta^+ \wedge T \right) \geq 1 - cTe^{-c\log(d)^2}
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    \begin{align*}
        \langle \vec{g}_t^\bot, \vec{w}_t^\star \rangle^2 &= (\langle \vec{w}^\star, \Et{\vec{g}_t^\bot} \rangle + Z_t)^2 \\
        &\leq 2(\langle \vec{w}^\star, \Et{\vec{g}_t^\bot} \rangle^2 + Z_t^2) \\
        &\leq \phi(m_t)^2(1 - m_t^2)^2 + O\left(\frac{d}{n_b}\right).
    \end{align*}
\end{proof}
...
\begin{align*}
    K(t_{\max}) &= \frac{\log(d)^{C - C_{\max}/2}}{C_{\max}\sqrt{d}} + C\gamma\log(d)^{C - C_{\max}}\frac d{n_b} \\
    &\leq \frac{\log(d)^{C - C_{\max}/2}}{C_{\max}\sqrt{d}} + \frac{Cc_\gamma}{\log(d)^{C_{\max}}} d^{-\frac{\ell}{2}} \\
    &\leq \frac{m_0}{4},
\end{align*}
