...
% \begin{equation}\label{2406:eq:initial_distribution_QM}
% \begin{split}
...
Consider the gradient descent dynamics defined on the hidden layer weights by eq.~\eqref{2406:eq:main:gd_update_weights}. We focus on the description of the time evolution of the correlation between the network's hidden layer weight and the target direction:
\begin{align}
...
\label{2406:thm:main:sgd_weak_recovery}
   Consider the projected SGD algorithm with square loss  (Eqs. \eqref{2406:eq:main:gd_update_weights}, \eqref{2406:eq:main:projected_sgd}), and suppose that Assumptions \ref{2406:assump:poly_growth}-\ref{2406:assump:init} hold. There exist absolute constants $c_\gamma, C_\gamma$ such that if
...
% , while is fixed to the optimal for fastest weak recovery for the other algorithms,  as characterized by Theorems.~(\ref{2406:thm:main:sgd_weak_recovery} ,\ref{2406:thm:main:no_yhat_weak_recovery}). 
} 
