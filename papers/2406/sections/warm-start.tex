...
\begin{figure*}
    ...
    \caption{\textbf{Exact asymptotic description:}
        ...
        The learning dynamics attains a plateau characterized by the noise variance \cite{saad1995line}. \textbf{\color{nodynamicscolor} Dynamics not defined:} The deterministic low-dimensional description of the eq.~\eqref{2406:eq:spherical_closed_form_ode} is not valid.
...
% The projected SGD dynamics over the weights in eq.~\eqref{2406:eq:main:gd_update_weights} describes a dynamical process for the order parameters that converges to a coupled set of low-dimensional deterministic equations. 
...
% Q^\bot = Q - M P^{-1}M^\top.
% \]
Then, there exists a constant $C$ independent from the input data dimension, such that the discrete stochastic process for the covariance \(\{\Omega_t\}_{t\in\mathbb{N}}\) in eq.~\eqref{2406:eq:main:covariance_def} induced by projected SGD dynamics is approximated by the deterministic covariance matrix \(\bar\Omega(t)\) with precision:
\begin{equation}
...
% \begin{itemize}
%     \item \(\Delta t \to 0\): Equations~\eqref{2406:eq:spherical_closed_form} converge to a system of ODEs; the solution of the equations \(\tilde \Omega(t)\) can be mapped in the sufficient statistics process with \(\Omega^t \equiv \tilde\Omega(t\Delta t)\);
%     \item \(\Delta t = O(1)\): Equations~\eqref{2406:eq:spherical_closed_form} define a \emph{deterministic} stochastic process; 
%     \item \(\Delta t \to +\infty\): Equations~\eqref{2406:eq:spherical_closed_form} diverge as well as the SGD weights dynamics.
% \end{itemize}
% Equations~\eqref{2406:eq:informal_ODE} describe the dynamics of non-projected SGD.
% When the first layer is trained with the update \eqref{2406:eq:spherical_gd_update_weight}, the sufficient statistics evolve following
% and the above considerations on \(\Delta t\) are valid.
% All these result can be formalized on a rigorous basis: we present the formal statements and their proof in Appendix~\ref{2406:sec:app:proofs}.
...
In Fig.~\ref{2406:fig:warm_start_pd}~(left) we summarize the results of Prop.~\ref{2406:prop:main:exact_asympt} in a compact phase diagram. 
...
\end{itemize}
In the right panel of Figure~\ref{2406:fig:warm_start_pd} we present a numerical investigation of three particular instances of the regimes presented above. The plot shows a comparison of numerical simulations versus the low-dimensional exact asymptotic characterization given in eqs.~\eqref{2406:eq:spherical_closed_form_ode}. The values of the learning rate and the batch size used for SGD training are varied to probe different regions of phase diagram~\ref{2406:fig:warm_start_pd}. 

\begin{remark}
When the target's leap index is $\ell>1$, the dynamic of SGD is dominated by a first extensive search phase to achieve weak recovery of the teacher direction (Thm.~\ref{2406:thm:main:sgd_weak_recovery}). 
% On the other hand, the ODE description for the dynamics in Prop.~\ref{2406:prop:main:exact_asympt} respects convergence bounds detailed in eq.~\eqref{2406:eq:main:time_horizon}. 
...
% \end{align}
In App.~\ref{2406:sec:app:hyperparams} we study the tightness of the exponential bound eq.~\eqref{2406:eq:main:time_horizon}; we argue supported by numerical illustrations that (on the practical side) the low dimensional ODE description is valid well beyond the extents of Prop.~\ref{2406:prop:main:exact_asympt}, as already observed by other works \cite{goldt2019dynamics,veiga2022phase} in different context. 
\end{remark}
