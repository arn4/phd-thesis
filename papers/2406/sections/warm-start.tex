...
\begin{figure*}
    ...
    \caption{\textbf{Exact asymptotic description:}
        ...
        % Sticking with \(\mu =\sfrac14\), we can show the accordance between simulated SGD and our simulation in {\color{allowedcolor} population flow regime \(\delta = -1\)}, {\color{olive} Saad\&Solla regime \(\delta = -\sfrac34\)}, and {\color{notallowedcolor} noise learning regime \(\delta = -\sfrac38\)}. All the simulation are run by \(d=10000\); we have some finite size effects in the population flow regime.
    }
