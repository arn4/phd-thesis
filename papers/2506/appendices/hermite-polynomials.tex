...
\[ \omega_p(\vec{x}) = \frac{1}{(2\pi)^{p/2}} \exp\left(-\frac{\norm{\vec{x}}^2}{2}\right), \]
...
\[ \langle (\cH_k)_{i_1\dots i_k}, (\cH_\ell)_{j_1\dots j_\ell} \rangle_\omega \neq 0 \, \text{if and only if} \, k=\ell \text{ and } (i_1, \dots, i_k) \text{ is a permutation of } (j_1, \dots, j_\ell) . \]
...
\begin{theorem}
    ...
    \[ \langle f, g \rangle_\omega = \sum_{k \geq 0} k!\,\langle C_k(f), C_k(g) \rangle \]
\end{theorem}
