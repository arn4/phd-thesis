\section{Further discussion on the positional-semantic transition} \label{2506:app:further-positional-semantic}
...
\subsection{SIE of model \texorpdfstring{\eqref{2506:eq:positional-semantic-label}}{\ref{2506:eq:positional-semantic-label}}}
...
\paragraph{The information exponent}
 We can also compute the \textit{information exponent} by looking at what rate \(m\) grows around initialization. We use \emph{spherical gradient descent} because we assumed the norm of the vector \(\vec w\) to be constant (as Ben Arous also does in his paper on Information Exponent):
 ...
 multiplying both sides by \(\sfrac{\vec{w}_\star}{d}\) we get
  \[
    m^{\nu+1} = \frac{m^\nu - \gamma \frac{\vec{w}_\star\cdot\nabla_{\vec{w}}\mathcal{L}}{d}}{\norm{\vec{w}^\nu - \gamma \nabla_{\vec{w}}\mathcal{L}}_2}\sqrt{d}.
 \]
...
\begin{equation} \label{2506:eq:m-update-normalized-gradient}
    ...
    \approx m^\nu - \gamma \frac{\vec{w}_\star\cdot\nabla_{\vec{w}}\mathcal{L}}{d} + m^\nu\gamma \frac{\vec{w}\cdot\nabla_w\mathcal{L}}{d},
\end{equation}
...
\[
 \frac{m^{\nu+1} - m^\nu}{\sfrac{\gamma}{d}} =
    -m\left.\frac{\partial^2 \mathcal{L}}{\partial m^2}\right|_{m,e=0} - e \left.\frac{\partial^2 \mathcal{L}}{\partial m\partial e}\right|_{m,e=0}.
 \]
 We can repeat the same derivation for finding the analogous equation for \(e\):
 \[
 \frac{e^{\nu+1} - e^\nu}{\sfrac{\gamma}{d}} =
    -e\left.\frac{\partial^2 \mathcal{L}}{\partial e^2}\right|_{m,e=0} - m \left.\frac{\partial^2 \mathcal{L}}{\partial m\partial e}\right|_{m,e=0}.
 \]
 Since the Hessian of the loss has always a negative eigenvalue, both these equations need \(\nu = O(d\log d)\) steps of population gradient descent at constant learning rate \(\gamma\) to escape a neighborhood of initialization, leading to \(\mathrm{SIE}=2\). This step count is not the sample complexity of online SGD: controlling the gradient noise forces \(\gamma = O(\sfrac{1}{\log d})\), and the same estimate \(\nu \asymp \sfrac{d}{\gamma}\cdot\log d\) then returns \(\mathcal{O}(d\log^2 d)\) --- the extra logarithmic factor that Theorem~\ref{2506:thm:sie} suppresses when it quotes the sharp \(\mathcal{O}(d\log d)\).

\subsection{Numerical experiments on the transition}
In this Appendix we would like to provide more details on the phase diagram of Figure~\ref{2506:fig:phase-diagram-positional-semantic}. 

In Figure~\ref{2506:fig:app:trajectories-d100-positional-semantic} we reproduce Figure~\ref{2506:fig:positional-semantic-trajectories-a1} for \(d=100\), clarifying the \emph{finite size} effects we claimed in the main text. Smaller values of \(d\) lead to a smoother transition, since the gradient flow assumption is less valid. In the limit \(d\to\infty\) we expect to see a step function.
\begin{figure}
    \centering
...
The yellow region in the phase diagram of Figure~\ref{2506:fig:phase-diagram-positional-semantic} is predicted to have a unique global semantic minimum, while SGD is aligned with the positional direction at initialization. The resulting trajectories are shown in Figure~\ref{2506:fig:app:phase-diagram-yellow-region}: the dynamics moves towards the minima direction, get stuck in the local flat (but not critical) region around \((e,m)=(1,0)\), and then it moves towards the global minima. In this region the convergence is very slow.
\begin{figure}
    \centering
...
The global minima of the population loss function does not transition from a positional to a semantic regime, but rather it smoothly move from the semantic to the positional regime as \(\omega\) increases. We leave the details and numerical experiments for future work.
