...
\begin{equation} \label{2506:eq:m-update-normalized-gradient}
    ...
    \approx m^\tau - \gamma \frac{\vec{w}_\star\cdot\nabla_{\vec{w}}\mathcal{L}}{d} + m^\tau\gamma \frac{\vec{w}\cdot\nabla_w\mathcal{L}}{d},
\end{equation}
...
\[
 ...
 \]
 Since the Hessian of the loss has always a negative eigenvalue, both these equations need \(\tau = O(d\log d)\) steps of population gradient descent at constant learning rate \(\gamma\) to escape a neighborhood of initialization, leading to \(\mathrm{SIE}=2\). This step count is not the sample complexity of online SGD: controlling the gradient noise forces \(\gamma = O(\sfrac{1}{\log d})\), and the same estimate \(\tau \asymp \sfrac{d}{\gamma}\cdot\log d\) then returns \(\mathcal{O}(d\log^2 d)\) --- the extra logarithmic factor that Theorem~\ref{2506:thm:sie} suppresses when it quotes the sharp \(\mathcal{O}(d\log d)\).
