...
\begin{theorem}\label{2506:thm:app:ben_arous_meta}
    Let $\nu_\eta$ be the weak recovery time of the ODE \eqref{2506:eq:app:ode_approx}, with the same initial conditions as the process \eqref{2506:eq:normalized-SGD-w}. Then for small enough $\eta$ and any $\delta > 0$, there exist constants $c(\delta), C(\delta)$ such that if
    \begin{equation}\label{2506:eq:app:gamma_optimal_value}
        \gamma = c(\delta)(d\nu_\eta)^{-1},
    \end{equation}
    then with probability at least $1 - \delta$
    \[ t_\eta^+ \leq C(\delta) d \nu_\eta^2. \]
    On the other hand, for any $t \leq C(\delta) d\nu_\eta^2$, if $\gamma \leq c(\delta)(dt)^{-1/2}$, then with probability $1 - \delta$
...
We are now ready to prove Theorem~\ref{2506:thm:sie} (as well as part of Theorem~\ref{2506:thm:sequencelength-gain}). In light of Theorem~\ref{2506:thm:app:ben_arous_meta}, it suffices to compute the hitting time $\nu_\eta$ of the tied dynamics \eqref{2506:eq:app:tied_dynamics}.
...
As a result, when $\gamma \leq \kappa(\delta)^{\sie-1} C^{-1}d^{-\frac{\sie}{2}+1}$, then for $0 \leq t \leq \nu_\eta$
\[ \frac{dm}{dt} \geq c' \cdot C_{\sie} \times (\ind_L, \dots, \ind_L) m^{\sie-1} \]
This implies:
\begin{itemize}
    \item when $\sie = 1$,
    \[ m(t) \geq m_0 + \frac{C_{\sie} \times (\ind_L, \dots, \ind_L)}{2}t, \quad \text{hence} \quad \nu_\eta \leq \frac{C'\eta}{C_{\sie} \times (\ind_L, \dots, \ind_L)}; \]
    \item when $\sie = 2$,
    \[ m(t) \geq m_0\exp\left(\frac{C_{\sie} \times (\ind_L, \dots, \ind_L)}{2}t\right), \quad \text{hence} \quad \nu_\eta \leq \frac{C'\ln(d)}{C_{\sie} \times (\ind_L, \dots, \ind_L)}; \]
    \item when $\sie \geq 3$,
    \[ m(t) \geq \left(m(0)^{2-\sie} - \frac{C_{\sie} \times (\ind_L, \dots, \ind_L)}{2}(\sie-2)t\right)^{-\frac1{\sie-2}},\] hence\[\nu_\eta \leq \frac{C'd^{\frac\sie2-1}}{C_{\sie} \times (\ind_L, \dots, \ind_L)}. \]
\end{itemize}
The bound on $\gamma$ above is always weaker (up to constant factors) than the bound $\gamma \leq C(d\nu_d)^{-1}$ of Theorem~\ref{2506:thm:app:ben_arous_meta}, and hence we always have
\[ t_\eta^+ \leq Cd\nu_\eta^2,\]
...
which by the same reasoning implies that the bounds on $\nu_\eta$ that we obtained are sharp up to constants.
...
Since $\norm{m} \geq m^{(1)}$ by the Cauchy-Schwarz inequality, the hitting time $\nu_{\eta, \text{untied}}$ is at most that of $m^{(1)}$, and hence
\[ \nu_{\eta, \text{untied}} \leq \frac{C}{\norm{C_\sie}} \begin{cases}
    \eta\sqrt{L} & \text{if} \sie=1 \\
    \log(d) & \text{if} \sie=2\\
    d^{\frac\sie2-1} & \text{if} \sie \geq 3
    \end{cases}
