...
\begin{theorem}\label{2506:thm:app:ben_arous_meta}
    Let $t_\eta$ be the weak recovery time of the ODE \eqref{2506:eq:app:ode_approx}, with the same initial conditions as the process \eqref{2506:eq:normalized-SGD-w}, and let $\nu^\mathrm{weak}_\eta$ be the number of gradient steps of Definition~\ref{2506:def:weak-recovery}. Then for small enough $\eta$ and any $\delta > 0$, there exist constants $c(\delta), C(\delta)$ such that if
    \begin{equation}\label{2506:eq:app:gamma_optimal_value}
        \gamma = c(\delta)(dt_\eta)^{-1},
    \end{equation}
    then with probability at least $1 - \delta$
    \[ \nu^\mathrm{weak}_\eta \leq C(\delta) d t_\eta^2. \]
    On the other hand, for any number of steps $\nu \leq C(\delta) dt_\eta^2$, if $\gamma \leq c(\delta)(d\nu)^{-1/2}$, then with probability $1 - \delta$
    \[ \nu^\mathrm{weak}_\eta \geq c(\delta)\nu \]
\end{theorem}
...
\begin{remark}
    ...
    \[ \nu^\mathrm{weak}_\eta(\gamma) \asymp \gamma^{-1}t_\eta. \]
\end{remark}
...
\[
    \resizebox{\linewidth}{!}{$\displaystyle
      \nabla_{\vw}^\bot \cL(X, y, f) = 2\sigma'\left( \frac{\langle \vw_{\text{tied}}, \Flatten(X)\rangle}{\sqrt{L}} \right)\left(\sigma'\left( \frac{\langle \vw_{\text{tied}}, \Flatten(X)\rangle}{\sqrt{L}} \right) - y(X)\right) \cdot \frac{\sum_{i} (I - \vw \vw^\top) \vec{x}_i^\parallel + \sum_i \vec{x}_i^\bot }{\sqrt{L}}
    $}
  \]
Importantly, the vector $x_i^\bot$ is independent from any of the prefactors, and has squared norm $d-2$, while the first vector is a fixed-dimensional Gaussian. As a result, we have
...
\[ \frac{L_0}2 \leq \E*{f_{\vw}(X)^2(y(X) - f_{\vw}(X))^2} \leq 2L_0, \]
which ends the proof.
\end{proof}

\subsection{Hitting time for the tied dynamics}

We are now ready to prove Theorem~\ref{2506:thm:sie} (as well as part of Theorem~\ref{2506:thm:sequencelength-gain}). In light of Theorem~\ref{2506:thm:app:ben_arous_meta}, it suffices to compute the hitting time $t_\eta$ of the tied dynamics \eqref{2506:eq:app:tied_dynamics}.
...
As a result, when $\gamma \leq \kappa(\delta)^{\sie-1} C^{-1}d^{-\frac{\sie}{2}+1}$, then for $0 \leq t \leq t_\eta$
\[ \frac{dm}{dt} \geq c' \cdot C_{\sie} \times (\ind_L, \dots, \ind_L) m^{\sie-1} \]
This implies:
\begin{itemize}
    \item when $\sie = 1$,
    \[ m(t) \geq m_0 + \frac{C_{\sie} \times (\ind_L, \dots, \ind_L)}{2}t, \quad \text{hence} \quad t_\eta \leq \frac{C'\eta}{C_{\sie} \times (\ind_L, \dots, \ind_L)}; \]
    \item when $\sie = 2$,
    \[ m(t) \geq m_0\exp\left(\frac{C_{\sie} \times (\ind_L, \dots, \ind_L)}{2}t\right), \quad \text{hence} \quad t_\eta \leq \frac{C'\ln(d)}{C_{\sie} \times (\ind_L, \dots, \ind_L)}; \]
    \item when $\sie \geq 3$,
    \[ m(t) \geq \left(m(0)^{2-\sie} - \frac{C_{\sie} \times (\ind_L, \dots, \ind_L)}{2}(\sie-2)t\right)^{-\frac1{\sie-2}},\] hence\[t_\eta \leq \frac{C'd^{\frac\sie2-1}}{C_{\sie} \times (\ind_L, \dots, \ind_L)}. \]
\end{itemize}
The bound on $\gamma$ above is always weaker (up to constant factors) than the bound $\gamma \leq C(dt_\eta)^{-1}$ of Theorem~\ref{2506:thm:app:ben_arous_meta}, and hence we always have
\[ \nu^\mathrm{weak}_\eta \leq Cdt_\eta^2,\]
...
which by the same reasoning implies that the bounds on $t_\eta$ that we obtained are sharp up to constants.
...
Since $\norm{m} \geq m^{(1)}$ by the Cauchy-Schwarz inequality, the hitting time $t_{\eta, \text{untied}}$ is at most that of $m^{(1)}$, and hence
\[ t_{\eta, \text{untied}} \leq \frac{C}{\norm{C_\sie}} \begin{cases}
    \eta\sqrt{L} & \text{if} \sie=1 \\
    \log(d) & \text{if} \sie=2\\
    d^{\frac\sie2-1} & \text{if} \sie \geq 3
    \end{cases}
