...
\begin{equation}\label{2605:eq:reproducing}
  ...
  \begin{equation}\label{2605:eq:spectral}
    T f = \sum_{i\geq 1} \lambda_i\langle f,e_i\ranglee_i\qquad\text{for all }f\in\mathcal{H}.
    \end{equation}
    \end{theorem}
  Trace-class and Hilbert--Schmidt operators are special cases of compact operators on which the spectrum is, respectively, absolutely summable ($\sum_i|\lambda_i|<\infty$) and square-summable ($\sum_i\lambda_i^2<\infty$).
    ...
    \begin{equation}\label{2605:eq:integral-op}
      ...
      \end{equation}
      Under the standard assumptions that $K$ is symmetric, continuous (or merely measurable) and square-integrable in the sense that $\int_{\mathcal{X}\times\mathcal{X}} K(\bx,\bx')^2d\mu(\bx)d\mu(\bx')<\infty$, the operator $T_K$ is self-adjoint and Hilbert--Schmidt (in particular compact) on $L^2(\mathcal{X},\mu)$.
      ...
      \begin{equation}\label{2605:eq:mercer}
        K(\bx,\bx')=\sum_{i\geq 1}\lambda_ie_i(\bx)e_i(\bx').
        \end{equation}
        ...
        \begin{equation}\label{2605:eq:rkhs-mercer}
          \mathcal{H}_K=\Bigl\{f=\sum_{i:\lambda_i>0}a_ie_i:\|f\|_{\mathcal{H}_K}^2:=\sum_{i:\lambda_i>0}\frac{a_i^2}{\lambda_i}<\infty\Bigr\},
          \end{equation}
...
\begin{equation}
G_{\ell-1}^{1/2}YG_{\ell-1}^{1/2}\bm\alpha_j = \lambda_j\bm\alpha_j, \quad j=1,\dots,k_\ell,
