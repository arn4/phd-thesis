...
\label{app:GD-neural}
In this section, our goal is to prove Proposition \ref{thm:layerwise-GD}, whose formal version is stated below as Theorem \ref{thm:neural-lofi}, i.e. show that under layerwise training with vanishing initialization, the early-time dynamics of each layer produces spikes along the linear direction $\hat{\bu}_\ell$ and aligns with the top eigenvectors of $\widehat{C}^{(\ell)}$, for a standard fully-connected architecture without skip connections.
...
\begin{equation}\label{eq:init-scales}
\alpha_m := \max_i \|\bw_{m,i}(0)\|, \qquad m=1,\dots,L,
\end{equation}
with  $\alpha_{L+1}:=\|a_L\|$. 

\begin{assumption}[Layerwise scale separation]\label{ass:scale-sep}
...
\begin{equation}\label{eq:sec2-linear-component}
\hat{\bu}_\ell
...
\begin{theorem}[Layerwise GD approximation]\label{thm:neural-lofi}
Under Assumptions~\ref{ass:scale-sep}--\ref{ass:nonlin}, there exist constants
$\tau,C>0$ such that the layerwise gradient descent run admits
horizons
\begin{equation}\label{eq:layerwise-horizons}
...
\begin{equation}\label{eq:effective-readout-fc}
\bar{a}_{\ell,i}
...
\begin{remark}
The three components in Equation~\eqref{eq:sec2-main-dynamics} have the following roles:
...
smaller by the scale hierarchy in Assumption~\ref{ass:scale-sep}.
\end{itemize}
...
\subsection{Proof of Theorem~\ref{thm:neural-lofi}}
...
\]
By the chain rule,
\begin{equation}\label{eq:full-grad}
...
The key observation is that, under Assumption~\ref{ass:scale-sep}, the activation
...
coefficient in~\eqref{eq:effective-readout-fc}. On the layerwise horizons
...
\begin{equation}\label{eq:Jac-correction}
...
\[
    ...
    \left(\sum_{m=\ell+1}^{L}\alpha_m\right),
\]
while \eqref{eq:readout-nondegenerate} gives the relative estimate
\begin{equation}\label{eq:readout-const-approx}
...
Assumption~\ref{ass:nonlin} give
\begin{equation}\label{eq:sec2-sigma-linearization}
\sigma'(u) = c_0 + c_1\,u + \omega_\sigma(u),\quad
|\omega_\sigma(u)|\leq \tfrac{1}{2}\|\sigma'''\|_\infty u^2 .
\end{equation}
Moreover, by~\eqref{eq:readout-const-approx},
...
Substituting these three estimates in~\eqref{eq:full-grad} yields
\[
\frac1n\sum_{\mu=1}^n
...
\eqref{eq:sec2-main-dynamics}.  Summing over \(0\le s<t\le T_\ell\) gives
\[
    \left\|
        \sum_{s=0}^{t-1}R_{\ell,i}(s)
    \right\|
    ...
    K\alpha_\ell^3,
\]
which is \eqref{eq:remainder-bound}.
\end{proof}
