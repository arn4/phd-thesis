...
\approx \etac_0 \hat{\bu}_\ell
+ \etac_1\widehat{C}^{(\ell)}\bw_{\ell,i}(t)+O(\norm{\bw_{\ell,i}(0)}^2),
\end{equation}
...
\begin{equation}\label{2605:eq:lofi-decomp}
    ...
    =
    \underbrace{W_\ell(0)\hat{\bm V}}_{\text{random transformation}} \times \qquad \underbrace{\exp(c_1\Lambdat)\hat{\bm V}^{\top}}_{\text{spectral filter}\to\text{top-eigenvector projection}}.
\end{equation}
